...
export default function ResultadoForm() {
  ...
  const handlePartidoSeleccionado = (e) => {
    const id_partido = e.target.value;
    const partido = partidos.find(p => String(p.id_partido) === String(id_partido));
    ...
    if (partido) {
      setFormData({
        ...
        resultados: [
          { ...formData.resultados[0], equipo_id: partido.equipo_local },
          { ...formData.resultados[1], equipo_id: partido.equipo_visitante }
        ]
      });
    } else {
      setFormData({
        ...
        partido_id: id_partido,
        resultados: [
          { ...formData.resultados[0], equipo_id: '' },
          { ...formData.resultados[1], equipo_id: '' }
        ]
      });
    }
  };
    ...
    <form onSubmit={handleSubmit} className="p-4 space-y-4 bg-white rounded shadow-md max-w-4xl mx-auto">
      ...
      <div className="mb-4">
        ...
        <select
          ...
          onChange={handlePartidoSeleccionado}
          required
        >
          <option value="">-- Selecciona un partido --</option>
          {partidos.map((p) => (
            <option key={p.id_partido} value={p.id_partido}>
              {new Date(p.fecha_partido).toLocaleDateString()} — {p.equipo_local_nombre} vs. {p.equipo_visitante_nombre}
            </option>
          ))}
        </select>
      </div>
